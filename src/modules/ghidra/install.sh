#!/usr/bin/env bash

set -euo pipefail
[[ "${WAR10CK_DEBUG:-0}" == "1" ]] && set -x

w_apt_install openjdk-21-jdk unzip

readonly GHIDRA_API_URL="https://api.github.com/repos/NationalSecurityAgency/ghidra"

# Fetch latest release metadata once - Ghidra embeds the SHA256 in the release
# body rather than publishing a checksums file, so we need the full JSON.
_tmpjson=$(mktemp --suffix=-ghidra-release.json)
w_download "${GHIDRA_API_URL}/releases/latest" "${_tmpjson}"
RELEASE_JSON=$(cat "${_tmpjson}")
rm -f "${_tmpjson}"

# Use two-stage grep to avoid variable-length lookbehind (not supported by GNU grep).
GHIDRA_DOWNLOAD_URL=$(printf '%s' "${RELEASE_JSON}" | grep "browser_download_url" | grep "\.zip" | cut -d '"' -f 4)
GHIDRA_SHA256=$(printf '%s' "${RELEASE_JSON}" | grep -i 'sha.256' | grep -oE '[a-f0-9]{64}' | head -1)

if [[ -z "${GHIDRA_DOWNLOAD_URL}" ]]; then
  w_log_error "Failed to resolve Ghidra download URL"
  exit 1
fi
if [[ -z "${GHIDRA_SHA256}" ]]; then
  w_log_error "Failed to parse Ghidra SHA256 from release notes"
  exit 1
fi

_tmpzip=$(mktemp --suffix=-ghidra.zip)
w_download "${GHIDRA_DOWNLOAD_URL}" "${_tmpzip}"

if ! w_verify_sha256 "${_tmpzip}" "${GHIDRA_SHA256}"; then
  rm -f "${_tmpzip}"
  exit 1
fi

for _old in /opt/ghidra*; do
  w_sudo_remove_dir "${_old}"
done
w_q sudo unzip -q "${_tmpzip}" -d /opt/
rm -f "${_tmpzip}"

GHIDRA_DIR=$(find /opt -maxdepth 1 -type d -name "ghidra_*_PUBLIC*" | sort | tail -n 1)
w_sudo_symlink "${GHIDRA_DIR}/ghidraRun" /usr/local/bin/ghidra
w_sudo_make_executable "${GHIDRA_DIR}/ghidraRun"

w_log_info "ghidra module installed."
